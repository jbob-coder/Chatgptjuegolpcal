...
def main() -> int:
    ...
    check("autorun folds combat result into fresh-instance signature", '"combat_exchange": exchange_signature' in test and "first_signature == second_signature" in test)
    ...
    check("combat-exchange verification gate is explicit", "Gate: HUNT01_BASIC_RUNTIME_AUTORUN_COMBAT_EXCHANGE_VERIFIED" in test)
    check("documentation marks this as CI verification rather than autoplay", "not player-facing autoplay" in doc.lower() and "fresh-instance" in doc.lower() and "combat exchange" in doc.lower())
    ...
    print("This gate verifies deterministic fresh-instance repeatability through one existing combat exchange; it does not claim player-facing autoplay, phone acceptance or sustained performance.")
