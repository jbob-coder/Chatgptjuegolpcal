...
LAST_VERIFIED_COMMIT: 5e0aac0d8857e5ce18889da3cd12dc83a248cc7e
STATIC_TEST_COUNT: 8
STATIC_TEST_PASS_COUNT: 8
QA_VALIDATOR_COUNT: 5
TOTAL_STATIC_CHECK_COUNT: 13
TOTAL_STATIC_PASS_COUNT: 13
...
Known limitation: Godot executable is unavailable in the current execution environment. The execution container cannot resolve github.com for a normal git clone, so static verification is run on connector-fetched reconstructed repository state. Git commit existence checks are performed only when a real git worktree is available and are explicitly reported as unavailable otherwise.
