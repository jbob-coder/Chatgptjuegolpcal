...
def main() -> int:
    ...
    for path in ROOT.rglob("*"):
        if not path.is_file() or path.suffix.lower() not in {".gd", ".tscn", ".godot", ".json", ".md", ".py"}:
            continue
        text = path.read_text(encoding="utf-8")
        require(not forbidden.search(text), f"forbidden/legacy term found in {path.relative_to(ROOT)}")
