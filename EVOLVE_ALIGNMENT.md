...
Status: ACTIVE PIXEL RPG / FIRST-PERSON + STARTING AREA ASSET PACK 001 BUILD-VERIFIED / SAFE VISUAL CONTINUATION
...
## Starting Area Asset Pack 001 exact-source checkpoint
...
Boundary:
1. upgrade only the enterable smith presentation assets;
2. preserve its current footprint, real doorway, wall-aligned collisions, roof visibility contract and interaction anchors;
3. keep first-person camera/input/HUD/targeting/combat/state owners untouched;
4. add a focused smith-visual regression without weakening Pack 004 physics/interaction tests;
5. no crafting-system expansion in this visual slice;
6. no phone/performance claims without device evidence.

NEXT THING: `PIXEL_RPG_STARTING_AREA_ASSET_PACK_002_SMITH_VISUAL`.
