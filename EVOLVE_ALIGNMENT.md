...
Status: ACTIVE PIXEL RPG / FIRST-PERSON + STARTING AREA ASSET PACK 002 SMITH BUILD-VERIFIED / SAFE VISUAL CONTINUATION
...
`PIXEL_RPG_STARTING_AREA_ASSET_PACK_003_ENVIRONMENT_DRESSING`.

Boundary:
1. improve only environmental dressing around the settlement/trail;
2. preserve all current gameplay coordinates and collision ownership;
3. prefer reusable presentation-only scenes for trees, rocks, vegetation, fences/banners and path-edge detail;
...
NEXT THING: `PIXEL_RPG_STARTING_AREA_ASSET_PACK_003_ENVIRONMENT_DRESSING`.
