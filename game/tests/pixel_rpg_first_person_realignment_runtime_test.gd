...
func _run() -> void:
	...
	_check("presentation init preserves Hunter logical transform", hunter != null and hunter.transform.is_equal_approx(hunter_before), str(hunter.transform if hunter != null else Transform3D.IDENTITY))
	...
	if camera != null and spring_arm != null:
		var camera_before := camera.global_position
		spring_arm.spring_length = 0.5
		await process_frame
		_check("camera no longer depends on chase-camera spring length", camera.global_position.distance_to(camera_before) <= 0.001, "%s -> %s" % [camera_before, camera.global_position])
