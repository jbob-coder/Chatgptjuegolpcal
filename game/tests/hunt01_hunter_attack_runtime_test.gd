...
func _run() -> void:
	print("Hunt-01 first Hunter attack / selected-part contact resolution")
	...
	if shell == null or movement == null or attack == null:
		_finish()
		return
	...
	_check("local dorsal protection routes before damage", String(first_resolution.get("protection_profile", "")) == "MINERALIZED_DORSAL_PLATE", str(first_resolution))
	...
	_check("damage is explicitly deferred to anatomy layer", String(handoff.get("status", "")) == "PENDING_ANATOMY_DAMAGE_RUNTIME", str(handoff))
	var readback := attack.call("get_last_resolution") as Dictionary
	_check("readback does not reroll committed attack", readback == first_resolution)
	...
	_check("Monster transform is unchanged because damage/behavior is not yet implemented", monster.global_transform.is_equal_approx(monster_before), str(monster.global_position))
...
func _finish() -> void:
	...
	print("Health/break/sever/status and phone/user acceptance are not claimed by this gate.")
