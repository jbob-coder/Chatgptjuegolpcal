...
const HEAD_SWEEP_CAPABILITY_ID := "CAP_M01_HEAD_ATTACK"
const HEAD_SWEEP_AP_COST := 2
const HEAD_SWEEP_STAMINA_COST := 14
const HEAD_SWEEP_BLOCK_IMPACT_DRAIN_STAMINA := 10
const HEAD_SWEEP_WORKING_MELEE_MAX_BODY_ENVELOPE_DISTANCE_M := 3.5
const HEAD_SWEEP_FIXTURE_STATUS := "PROVISIONAL_FIRST_SLICE_MUDCREST_HEAD_SWEEP_CONTROL_FIXTURE"
const HEAD_SWEEP_STRUCTURAL_CAPABILITY_STATUS := "PROVISIONAL_BASELINE_HEAD_HORNS_USABLE_NO_BREAK_STATE_RUNTIME"

# Compatibility aliases retained for the already-verified Head Sweep static/runtime gates.
...
const TAIL_SWEEP_BLOCK_IMPACT_DRAIN_STAMINA := 14
const TAIL_SWEEP_WORKING_MAX_BODY_ENVELOPE_DISTANCE_M := 8.0
const TAIL_SWEEP_MAX_FORWARD_DOT := 0.25
const TAIL_SWEEP_FIXTURE_STATUS := "USER_AUTHORIZED_CREATIVE_CANON_PROVISIONAL_TAIL_SWEEP_CONTROL_FIXTURE"
const TAIL_SWEEP_STRUCTURAL_CAPABILITY_STATUS := "PROVISIONAL_TAIL_DISTAL_ATTACHED_BASELINE_NO_SEVER_THRESHOLD_RUNTIME"
...
const TRACE_HEAD_SWEEP_TELEGRAPH := "MUDCREST_HEAD_SWEEP_TELEGRAPH_EMITTED"
const TRACE_HEAD_SWEEP_RESOLUTION := "MUDCREST_HEAD_SWEEP_RESOLUTION_HANDOFF_COMMITTED"
const TRACE_TAIL_SWEEP_TELEGRAPH := "MUDCREST_TAIL_SWEEP_TELEGRAPH_EMITTED"
const TRACE_TAIL_SWEEP_RESOLUTION := "MUDCREST_TAIL_SWEEP_RESOLUTION_HANDOFF_COMMITTED"
...
var _pivot_center := Vector2.ZERO
var _pivot_radius := 0.0
...
func initialize(world: Node3D, shell: Node, reaction: Node, anatomy: Node, encounter_record: Dictionary) -> bool:
	...
	_record_trace("MUDCREST_ATTACK_RUNTIME_READY", {
		...
		"head_sweep_ap_cost": HEAD_SWEEP_AP_COST,
		"head_sweep_stamina_cost": HEAD_SWEEP_STAMINA_COST,
		"tail_sweep_ap_cost": TAIL_SWEEP_AP_COST,
		"tail_sweep_stamina_cost": TAIL_SWEEP_STAMINA_COST,
		"head_fixture_status": HEAD_SWEEP_FIXTURE_STATUS,
		"tail_fixture_status": TAIL_SWEEP_FIXTURE_STATUS,
...
func _process(_delta: float) -> void:
	if _active_attack.is_empty() or String(_active_attack.get("state", "")) != "WAITING_REACTION_DECISION":
		return
	...
	if reaction_state == "COMMITTED_WAITING_SOURCE_RESOLUTION" or reaction_state == "DECLINED_WAITING_SOURCE_RESOLUTION":
		_resolve_active_attack(active_window)
...
func _load_manifest_context() -> bool:
	...
	if pivot_center.size() < 3:
		return false
	_pivot_center = Vector2(float(pivot_center[0]), float(pivot_center[2]))
	_pivot_radius = float(pivot.get("radius", 0.0))
	return _authored_forward_xz.length() > 0.0 and _pivot_radius > 0.0
...
func _head_capability_state() -> Dictionary:
	...
	return {
		...
		"capability_id": HEAD_SWEEP_CAPABILITY_ID,
		"profile": "GORE_SWEEP",
		"damage_channels": ["PIERCING", "IMPACT"],
		"structural_capability_status": HEAD_SWEEP_STRUCTURAL_CAPABILITY_STATUS,
	}
...
func _tail_capability_state() -> Dictionary:
	...
	var explicit_attachment := String(tail_state.get("distal_attachment_state", ""))
	if explicit_attachment == "SEVERED" or explicit_attachment == "DETACHED":
		return {"available": false, "reason": "TAIL_DISTAL_NOT_ATTACHED"}
	return {
		"available": true,
		...
		"sever_threshold_status": "DEFERRED_NOT_REQUIRED_FOR_INTACT_TAIL_ATTACK",
	}
...
func _pivot_clearance_state() -> Dictionary:
	if _monster == null:
		return {"legal": false, "reason": "MONSTER_MISSING"}
	var monster_xz := Vector2(_monster.global_position.x, _monster.global_position.z)
	var offset := monster_xz.distance_to(_pivot_center)
	if offset > _pivot_radius:
		return {"legal": false, "reason": "MONSTER_OUTSIDE_AUTHORED_PIVOT_CLEARANCE", "offset_m": offset, "pivot_radius_m": _pivot_radius}
	return {"legal": true, "reason": "AUTHORED_PIVOT_CLEARANCE_AVAILABLE", "offset_m": offset, "pivot_radius_m": _pivot_radius}
...
func get_head_sweep_legality() -> Dictionary:
	...
	if envelope_distance > HEAD_SWEEP_WORKING_MELEE_MAX_BODY_ENVELOPE_DISTANCE_M:
		return {"legal": false, "reason": "OUT_OF_WORKING_MELEE", "body_envelope_distance_m": envelope_distance, "max_body_envelope_distance_m": HEAD_SWEEP_WORKING_MELEE_MAX_BODY_ENVELOPE_DISTANCE_M}
	...
	if int(resources.get("ap", 0)) < HEAD_SWEEP_AP_COST:
		return {"legal": false, "reason": "INSUFFICIENT_AP", "resources": resources}
	if int(resources.get("stamina", 0)) < HEAD_SWEEP_STAMINA_COST:
		return {"legal": false, "reason": "INSUFFICIENT_STAMINA", "resources": resources}
	return {"legal": true, "reason": "LEGAL_M01_HEAD_SWEEP_GORE", "capability": capability.duplicate(true), "body_envelope_distance_m": envelope_distance, "max_body_envelope_distance_m": HEAD_SWEEP_WORKING_MELEE_MAX_BODY_ENVELOPE_DISTANCE_M, "bearing_dot": bearing_dot, "bearing_class": "FRONT_OR_FRONT_FLANK_AUTHORED_FORWARD_HEMISPHERE", "line_of_effect": line, "ap_cost": HEAD_SWEEP_AP_COST, "stamina_cost": HEAD_SWEEP_STAMINA_COST}
...
func get_tail_sweep_legality() -> Dictionary:
	...
	if not bool(capability.get("available", false)):
		return {"legal": false, "reason": String(capability.get("reason", "TAIL_CAPABILITY_UNAVAILABLE"))}
	var envelope_distance := _distance_to_body_envelope()
	if envelope_distance > TAIL_SWEEP_WORKING_MAX_BODY_ENVELOPE_DISTANCE_M:
		return {"legal": false, "reason": "OUT_OF_TAIL_SWEEP_REACH", "body_envelope_distance_m": envelope_distance, "max_body_envelope_distance_m": TAIL_SWEEP_WORKING_MAX_BODY_ENVELOPE_DISTANCE_M}
	var bearing_dot := _bearing_dot()
	...
	var pivot := _pivot_clearance_state()
	if not bool(pivot.get("legal", false)):
		return {"legal": false, "reason": "HINDQUARTER_PIVOT_BLOCKED", "pivot_clearance": pivot}
	var line := _line_of_effect()
	if not bool(line.get("clear", false)):
		return {"legal": false, "reason": "TAIL_SWEEP_ARC_BLOCKED", "line_of_effect": line, "pivot_clearance": pivot}
	var resources: Dictionary = _shell.call("get_resource_state", MONSTER_COMBATANT_ID)
	...
	return {
		...
		"max_body_envelope_distance_m": TAIL_SWEEP_WORKING_MAX_BODY_ENVELOPE_DISTANCE_M,
		"bearing_dot": bearing_dot,
		"bearing_class": "REAR_OR_FLANK_DOT_AT_OR_BELOW_PROVISIONAL_THRESHOLD",
		"max_forward_dot": TAIL_SWEEP_MAX_FORWARD_DOT,
		"pivot_clearance": pivot,
		"line_of_effect": line,
		"ap_cost": TAIL_SWEEP_AP_COST,
		"stamina_cost": TAIL_SWEEP_STAMINA_COST,
		"range_selection_authority": "USER_AUTHORIZED_CREATIVE_CANON_FROM_EXISTING_8M_AUTHORED_PIVOT_RADIUS",
	}
...
func begin_monster_activation(actor_id: String, round_id: int) -> bool:
	...
	if bool(tail_legality.get("legal", false)):
		return _begin_attack(round_id, TAIL_SWEEP_ATTACK_ID, tail_legality)
	var head_legality := get_head_sweep_legality()
	if bool(head_legality.get("legal", false)):
		return _begin_attack(round_id, HEAD_SWEEP_ATTACK_ID, head_legality)
	_record_trace("MUDCREST_TAIL_SWEEP_SKIPPED", {"round_id": round_id, "reason": String(tail_legality.get("reason", "TAIL_SWEEP_ILLEGAL")), "legality": tail_legality.duplicate(true)})
	_record_trace("MUDCREST_HEAD_SWEEP_SKIPPED", {"round_id": round_id, "reason": String(head_legality.get("reason", "HEAD_SWEEP_ILLEGAL")), "legality": head_legality.duplicate(true)})
	return bool(_shell.call("complete_external_activation", MONSTER_COMBATANT_ID, "WAIT_NO_ATTACK_RUNTIME"))

func _attack_ap_cost(attack_id: String) -> int:
	return TAIL_SWEEP_AP_COST if attack_id == TAIL_SWEEP_ATTACK_ID else HEAD_SWEEP_AP_COST

func _attack_stamina_cost(attack_id: String) -> int:
	return TAIL_SWEEP_STAMINA_COST if attack_id == TAIL_SWEEP_ATTACK_ID else HEAD_SWEEP_STAMINA_COST

func _attack_block_impact_drain(attack_id: String) -> int:
	return TAIL_SWEEP_BLOCK_IMPACT_DRAIN_STAMINA if attack_id == TAIL_SWEEP_ATTACK_ID else HEAD_SWEEP_BLOCK_IMPACT_DRAIN_STAMINA

func _attack_fixture_status(attack_id: String) -> String:
	return TAIL_SWEEP_FIXTURE_STATUS if attack_id == TAIL_SWEEP_ATTACK_ID else HEAD_SWEEP_FIXTURE_STATUS

func _attack_trace_prefix(attack_id: String) -> String:
	return "MUDCREST_TAIL_SWEEP" if attack_id == TAIL_SWEEP_ATTACK_ID else "MUDCREST_HEAD_SWEEP"

func _attack_telegraph_text(attack_id: String) -> String:
	if attack_id == TAIL_SWEEP_ATTACK_ID:
		return "Mudcrest Raker plants its hindquarters, counter-rotates its torso, and coils the mineralized tail across the rear/flank arc."
	return "Mudcrest Raker cocks its horned head, loads the forequarter, and sweeps across the front arc."

func _begin_attack(round_id: int, attack_id: String, legality: Dictionary) -> bool:
	_attack_sequence += 1
	var ap_cost := _attack_ap_cost(attack_id)
	var stamina_cost := _attack_stamina_cost(attack_id)
	...
	if not bool(_shell.call("try_commit_cost", MONSTER_COMBATANT_ID, action_source_id, ap_cost, stamina_cost)):
		_record_trace("%s_COMMIT_REJECTED" % _attack_trace_prefix(attack_id), {"round_id": round_id, "action_sequence": _attack_sequence, "reason": "RESOURCE_COMMIT_REJECTED_AFTER_VALIDATION"})
		return bool(_shell.call("complete_external_activation", MONSTER_COMBATANT_ID, "WAIT_NO_ATTACK_RUNTIME"))
	var telegraph_text := _attack_telegraph_text(attack_id)
	...
	if not bool(opened.get("success", false)):
		_record_trace("%s_REACTION_OPEN_FAILED" % _attack_trace_prefix(attack_id), {"round_id": round_id, "action_sequence": _attack_sequence, "reaction_result": opened.duplicate(true)})
		push_error("Committed Mudcrest attack could not open its required reaction window.")
		return bool(_shell.call("complete_external_activation", MONSTER_COMBATANT_ID, "WAIT_NO_ATTACK_RUNTIME"))
	...
	_active_attack = {
		...
		"attack_id": attack_id,
		"profile": String(capability.get("profile", "")),
		"damage_channels": (capability.get("damage_channels", []) as Array).duplicate(true),
		...
		"fixture_status": _attack_fixture_status(attack_id),
		"structural_capability_status": String(capability.get("structural_capability_status", "")),
		"ap_cost": ap_cost,
		"stamina_cost": stamina_cost,
		"standard_block_impact_drain_stamina": _attack_block_impact_drain(attack_id),
	}
	_show_telegraph_visual(attack_id)
	_record_trace("%s_TELEGRAPH_EMITTED" % _attack_trace_prefix(attack_id), _active_attack)
	return true

func _resolve_active_attack(reaction_window: Dictionary) -> void:
	...
	var attack_id := String(_active_attack.get("attack_id", ""))
	...
	var block_impact_drain := _attack_block_impact_drain(attack_id)
	var damage_handoff := {
		...
		"attack_id": attack_id,
		"attack_profile": String(_active_attack.get("profile", "")),
		"damage_channels": (_active_attack.get("damage_channels", []) as Array).duplicate(true),
		"reaction_id": reaction_id,
		"contact_class": contact_class,
		"hit_quality": hit_quality,
		"protection_route": protection_route,
		"standard_block_impact_drain_stamina": block_impact_drain if block_committed else 0,
	...
	if not bool(defense_consequence.get("success", false)):
		push_error("Mudcrest attack could not resolve the Hunter defense consequence.")
	var wound_contact: Dictionary = {}
	if attack_id == TAIL_SWEEP_ATTACK_ID:
		wound_contact = _wound_contact.call("resolve_tail_sweep_consequence", damage_handoff, defense_consequence)
	else:
		wound_contact = _wound_contact.call("resolve_head_sweep_consequence", damage_handoff, defense_consequence)
	if not bool(wound_contact.get("success", false)):
		push_error("Mudcrest attack could not classify its wound/contact consequence.")
	var resolution := {
		"success": true,
		...
		"hit_quality": hit_quality,
		"protection_route": protection_route,
		"fixture_status": String(_active_attack.get("fixture_status", "")),
		"structural_capability_status": String(_active_attack.get("structural_capability_status", "")),
	...
	if String(defeat_handoff.get("status", "")) == "PENDING_HUNTER_DEFEAT_OUTCOME_RUNTIME":
		...
		if not bool(outcome_consequence.get("success", false)):
			push_error("Mudcrest attack reached zero Hunter Health but the encounter outcome transaction failed.")
	...
	_record_trace("%s_RESOLUTION_HANDOFF_COMMITTED" % _attack_trace_prefix(attack_id), _last_resolution)
	_active_attack.clear()
	if bool(outcome_consequence.get("encounter_terminal", false)):
		return
	var completion_reason := "TAIL_SWEEP_DEFENSE_CONSEQUENCE_COMMITTED" if attack_id == TAIL_SWEEP_ATTACK_ID else "HEAD_SWEEP_DEFENSE_CONSEQUENCE_COMMITTED"
	if not bool(_shell.call("complete_external_activation", MONSTER_COMBATANT_ID, completion_reason)):
		push_error("Mudcrest attack resolved but the shell could not complete the Monster activation.")
...
func _show_telegraph_visual(attack_id: String) -> void:
	_hide_telegraph_visual()
	if _world == null or _monster == null or _hunter == null:
		return
	var scene := TAIL_SWEEP_TELEGRAPH_SCENE if attack_id == TAIL_SWEEP_ATTACK_ID else HEAD_SWEEP_TELEGRAPH_SCENE
	if scene == null:
		return
...
func get_attack_id() -> String:
	# Compatibility surface retained for the already-verified Head Sweep tests.
	return HEAD_SWEEP_ATTACK_ID

func get_supported_attack_ids() -> Array[String]:
	return [HEAD_SWEEP_ATTACK_ID, TAIL_SWEEP_ATTACK_ID]
...
func get_working_melee_limit_m() -> float:
	return HEAD_SWEEP_WORKING_MELEE_MAX_BODY_ENVELOPE_DISTANCE_M

func get_tail_sweep_working_limit_m() -> float:
	return TAIL_SWEEP_WORKING_MAX_BODY_ENVELOPE_DISTANCE_M

func get_tail_sweep_max_forward_dot() -> float:
