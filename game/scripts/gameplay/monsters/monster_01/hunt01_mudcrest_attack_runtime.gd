...
func _resolve_active_head_sweep(reaction_window: Dictionary) -> void:
	...
	# Defeat is consumed only after health, species wound/status classification,
	# and reaction closure have all completed. This preserves the contract's
	# authoritative resolution boundary before scheduler termination.
